...
type Config struct {
	...
	Quiet bool    `ini:"quiet" cfg:"quiet" cfgDefault:"true" cfgHelper:"Disable sound."`
}
...
func main() {
	...
	// Instead, we use the config package to parse command-line flags and the ini file
	config.PrefixEnv = "NEKO"
	config.File = "neko.ini"
	config.Parse(cfg) // This will parse the config and command-line flags
